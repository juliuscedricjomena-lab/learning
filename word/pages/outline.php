
<div class="top-actions">
    <div class="action-btn action-btn-radius action-btn-folder">
        <img src="<?php echo BASE_PATH; ?>images/icons/folder.png" alt="Folder">
    </div>
    <div class="action-btn action-btn-radius action-btn-gear">
        <img src="<?php echo BASE_PATH; ?>images/icons/gear.png" alt="Settings">
    </div>
</div>
<div class="page-content">
    <div class="category-grid">
        <a href="<?php echo BASE_PATH; ?>?page=activity1" class="category-card">
            <div class="category-badge">Activity 1</div>
            <img src="<?php echo BASE_PATH; ?>images/category-card.png" alt="Category" class="category-bg">
            <div class="category-content">
                <div class="category-left">
                    <div class="category-title">Look &<br>&emsp;&emsp;Tell</div>
                </div>
                <div class="category-right">
                    <img src="<?php echo BASE_PATH; ?>images/look-and-tell-avatar.png" alt="Avatar">
                </div>
                <img src="<?php echo BASE_PATH; ?>images/next-btn.png" alt="Next" class="category-next-btn">
            </div>
        </a>
        <a href="<?php echo BASE_PATH; ?>?page=activity2" class="category-card">
            <div class="category-badge">Activity 2</div>
            <img src="<?php echo BASE_PATH; ?>images/category-card.png" alt="Category" class="category-bg">
            <div class="category-content">
                <div class="category-left">
                    <div class="category-title">Listen &<br>&emsp;&emsp;Match</div>
                </div>
                <div class="category-right">
                    <img src="<?php echo BASE_PATH; ?>images/listen-and-match-avatar.png" alt="Avatar">
                </div>
                <img src="<?php echo BASE_PATH; ?>images/next-btn.png" alt="Next" class="category-next-btn">
            </div>
        </a>
        <a href="<?php echo BASE_PATH; ?>?page=activity3" class="category-card">
            <div class="category-badge">Activity 3</div>
            <img src="<?php echo BASE_PATH; ?>images/category-card.png" alt="Category" class="category-bg">
            <div class="category-content">
                <div class="category-left">
                    <div class="category-title">Copy<br>&emsp;&emsp;Cat Say</div>
                </div>
                <div class="category-right">
                    <img src="<?php echo BASE_PATH; ?>images/copy-cat-say-avatar.png" alt="Avatar">
                </div>
                <img src="<?php echo BASE_PATH; ?>images/next-btn.png" alt="Next" class="category-next-btn">
            </div>
        </a>
        <a href="<?php echo BASE_PATH; ?>?page=activity4" class="category-card">
            <div class="category-badge">Activity 4</div>
            <img src="<?php echo BASE_PATH; ?>images/category-card.png" alt="Category" class="category-bg">
            <div class="category-content">
                <div class="category-left">
                    <div class="category-title">Sound<br>&emsp;&emsp;Choice</div>
                </div>
                <div class="category-right">
                    <img src="<?php echo BASE_PATH; ?>images/sound-choice-avatar.png" alt="Avatar">
                </div>
                <img src="<?php echo BASE_PATH; ?>images/next-btn.png" alt="Next" class="category-next-btn">
            </div>
        </a>
        <a href="<?php echo BASE_PATH; ?>?page=activity5" class="category-card">
            <div class="category-badge">Activity 5</div>
            <img src="<?php echo BASE_PATH; ?>images/category-card.png" alt="Category" class="category-bg">
            <div class="category-content">
                <div class="category-left">
                    <div class="category-title">Say &<br>&emsp;&emsp;Play</div>
                </div>
                <div class="category-right">
                    <img src="<?php echo BASE_PATH; ?>images/say-and-play-avatar.png" alt="Avatar">
                </div>
                <img src="<?php echo BASE_PATH; ?>images/next-btn.png" alt="Next" class="category-next-btn">
            </div>
        </a>
        <a href="<?php echo BASE_PATH; ?>?page=activity6" class="category-card">
            <div class="category-badge">Activity 6</div>
            <img src="<?php echo BASE_PATH; ?>images/category-card.png" alt="Category" class="category-bg">
            <div class="category-content">
                <div class="category-left">
                    <div class="category-title">Final Say</div>
                </div>
                <div class="category-right">
                    <img src="<?php echo BASE_PATH; ?>images/final-say-avatar.png" alt="Avatar">
                </div>
                <img src="<?php echo BASE_PATH; ?>images/next-btn.png" alt="Next" class="category-next-btn">
            </div>
        </a>
    </div>
</div>
